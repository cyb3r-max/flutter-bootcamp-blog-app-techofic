...
class BlogController extends StateNotifier<BaseState> {
  ...
  Future updateToFavBlog({blogId, isFavorite}) async {
    state = const LoadingState();

    dynamic requestBody = {'id': blogId, 'is_favorite': isFavorite};
    dynamic responseBody;

    try {
      responseBody = await Network.handleResponse(
        await Network.postRequest(API.addToFavoriteBlogs, requestBody),
      );
      if (responseBody != null) {
        BlogModel blogData = ref!.read(blogDetailsProvider.notifier).blogModel!;
        blogData.id = blogId;
        blogData.isFavorite = isFavorite;
